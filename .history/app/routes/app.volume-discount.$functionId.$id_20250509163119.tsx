...
export const action: ActionFunction = async ({ request, params }) => {
  ...
  const { functionId, discountId } = params; // cần truyền thêm discountId từ route
  const { admin } = await authenticate.admin(request);
  ...
  const baseDiscount = {
    id: discountId, // thêm id để cập nhật
    functionId,
    title,
    combinesWith,
  ...
  if (method === DiscountMethod.Code) {
    ...
    const response = await admin.graphql(
      `#graphql
      mutation UpdateCodeDiscount($discount: DiscountCodeAppInput!) {
        discountUpdate: discountCodeAppUpdate(codeAppDiscount: $discount) {
          codeAppDiscount {
            discountId
          }
          userErrors {
    ...
    const response = await admin.graphql(
      `#graphql
      mutation UpdateAutomaticDiscount($discount: DiscountAutomaticAppInput!) {
        discountUpdate: discountAutomaticAppUpdate(automaticAppDiscount: $discount) {
