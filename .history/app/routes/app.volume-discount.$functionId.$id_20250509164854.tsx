...
export const action: ActionFunction = async ({ request, params }) => {
  ...
  // Trước tiên, lấy thông tin discount hiện tại để lấy metafield ID
    ...
    discountUpdateResponse = await admin.graphql(
      ...
      }`,
      {
        variables: {
          ...
          discount: baseCodeDiscount, // Không bao gồm metafields trong update discount
        },
      },
    );
    ...
    discountUpdateResponse = await admin.graphql(
      ...
      }`,
      {
        variables: {
          ...
          discount: baseDiscount, // Không bao gồm metafields trong update discount
        },
      },
    );
  ...
  // Nếu cập nhật discount thành công, tiếp theo cập nhật metafield
  if (errors && errors.length > 0) {
    return json({ errors });
  }

  // Cập nhật metafield riêng
  if (metafieldId) {
    ...
    const metafieldErrors =
      metafieldResponseJson.data.metafieldUpdate?.userErrors;
