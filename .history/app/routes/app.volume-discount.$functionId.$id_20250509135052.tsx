...
import { useForm, useField, Field } from "@shopify/react-form";
...
export const loader: LoaderFunction = async ({ params, request }) => {
  ...
  if (!id || !functionId) {
    return json({
      discount: null,
      error: "Missing discount ID or function ID",
    });
  }
  ...
  try {
    // Query the discount data and metafield
    ...
    // Query discount details based on type
    let discount;
    if (discountType === "code") {
      const codeResponse = await admin.graphql(
        `#graphql
        query GetCodeDiscountDetails($id: ID!) {
          codeDiscountNode: discountNode(id: $id) {
            codeDiscount {
              ... on DiscountCodeApp {
                title
                startsAt
      ...
      const codeDiscountNode = codeResponseJson.data?.codeDiscountNode;

      if (!codeDiscountNode || !codeDiscountNode.codeDiscount) {
        return json({
          discount: null,
          error: "Code discount details not found",
        });
      }

      discount = codeDiscountNode.codeDiscount;
      ...
      const automaticResponse = await admin.graphql(
        `#graphql
        query GetAutomaticDiscountDetails($id: ID!) {
          automaticDiscountNode: discountNode(id: $id) {
            automaticDiscount {
      ...
      const automaticResponseJson = await automaticResponse.json();
      const automaticDiscountNode =
        automaticResponseJson.data?.automaticDiscountNode;

      if (!automaticDiscountNode || !automaticDiscountNode.automaticDiscount) {
        return json({
          discount: null,
          error: "Automatic discount details not found",
        });
      }

      discount = automaticDiscountNode.automaticDiscount;
    ...
    return json({ discount: null, error: "Failed to fetch discount details" });
  }
};
...
export const action: ActionFunction = async ({ params, request }) => {
  ...
  if (!id || !functionId) {
    return json({
      errors: [
        { message: "Missing discount ID or function ID", field: ["params"] },
      ],
    });
  }
...
export default function VolumeEdit() {
  ...
  const {
    ...
    fields: {
      ...
      usageLimit: useField(
        discountData?.usageLimit === undefined
          ? null
          : String(discountData?.usageLimit),
      ),
      appliesOncePerCustomer: useField(
        discountData?.appliesOncePerCustomer || false,
      ),
      startDate: useField(
        discountData?.startsAt ? new Date(discountData.startsAt) : todaysDate,
      ),
      endDate: useField(
        discountData?.endsAt ? new Date(discountData.endsAt) : null,
      ),
      configuration: {
        quantity: useField(String(discountData?.configuration?.quantity || 1)),
        percentage: useField(
          String(discountData?.configuration?.percentage || 0),
        ),
      },
    },
    onSubmit: async (form) => {
          ...
          <Form method="post">
            <BlockStack align="space-around" gap="200">
              ...
              <ActiveDatesCard
                ...
                endDate={endDate as unknown as Field<string | null>}
        ...
        <Layout.Section variant="oneThird">
          <SummaryCard
            ...
            activeDates={{
              ...
              endDate: endDate.value as unknown as string | null,
            }}
          />
        </Layout.Section>
        <Layout.Section>
