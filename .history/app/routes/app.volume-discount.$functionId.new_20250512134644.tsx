    ...
    if (collections) {
      const collectionList = collections.map((item) => {
        return {
          ...
          collectionImage: item.image?.originalSrc,
        };
      });

      console.log("collectionList", collectionList);
      setCollectionState(collectionList);
    }
  }
  ...
  const {
    ...
    onSubmit: async (form) => {
      const discount = {
        ...
        endsAt: form.endDate,
        configuration: {
          quantity: tier.reduce((acc, item) => {
            acc[`tier0${item}`] = parseInt(
              form.configuration.quantity?.[`tier0${item}`],
            );
            return acc;
          }, {}),
          percentage: tier.reduce((acc, item) => {
            acc[`tier0${item}`] = parseInt(
              form.configuration.percentage?.[`tier0${item}`],
            );
            return acc;
          }, {}),
        },
        ...
        productId:
          selectionType === "product"
            ? formState?.map((item) => item.productId)
            : [],
        collectionId:
          selectionType === "collection"
            ? collectionState?.map((item) => item.collectionId)
            : [],
      };
              ...
              <Card>
                <Card.Section>
                  <BlockStack gap="400">
                    ...
                    <InlineStack gap="300">
                      <Button
                        pressed={selectionType === "product"}
                        onClick={() => setSelectionType("product")}
                      >
                        Products
                      </Button>
                      <Button
                        pressed={selectionType === "collection"}
                        onClick={() => setSelectionType("collection")}
                      >
                        Collections
                      </Button>
                    </InlineStack>
                        ...
                        return (
                          ...
                          >
                            ...
                            <TextField
                              ...
                              autoComplete="on"
                              {...configuration.quantity?.[`tier0${item}`]}
                            />
                            <TextField
                              label="Discount percentage"
                              autoComplete="on"
                              {...configuration.percentage?.[`tier0${item}`]}
                              suffix="%"
                            />
                          </Grid.Cell>
                        );
