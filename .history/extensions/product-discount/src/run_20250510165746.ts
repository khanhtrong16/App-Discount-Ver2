...
export function run(input: RunInput): FunctionRunResult {
  ...
  console.log(
    "configuration đây: ",
    JSON.stringify(configuration.quantity.tier01, null, 2),
  );
  ...
  const DiscountList = productCartList
    .map((line) => {
      if (line.quantity >= configuration.quantity.tier03) {
        return { percentage: configuration.percentage.tier03, id: line.id };
      } else if (line.quantity >= configuration.quantity.tier02) {
        return { percentage: configuration.percentage.tier02, id: line.id };
      } else if (line.quantity >= configuration.quantity.tier01) {
        return { percentage: configuration.percentage.tier01, id: line.id };
      }
      return null;
    })
