...
export function run(input: RunInput): FunctionRunResult {
  ...
  let productCartId = "";
  const productList = productCarts.filter((line) => {
    productCartId = line.merchandise.product.id;
    const test = ArrayProductId.map((item: any) => item == productCartId);
    console.log("test : ", test);
    return ArrayProductId.map((item: any) => item == productCartId);
